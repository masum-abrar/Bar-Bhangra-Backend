...
export const createContact = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      let { name, email, subject, message, isActive } = req.body;
      ...
      if (!req.file) {
        ...
        //create brand
        const newNewsletter = await prisma.contact.create({
          data: {
            name,
            email,
            subject,
        ...
        if (newNewsletter) {
          return res
            .status(200)
            .json(jsonResponse(true, "Message has been sent.", newNewsletter));
        }
      }
